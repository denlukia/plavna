...
const XMLHttpRequestShim = class XMLHttpRequest extends Dispatch {
	...
	getAllResponseHeaders() {
		if (this[sErrored] || this.readyState < XMLHttpRequest.HEADERS_RECEIVED) return '';
		...
		fetch(this.url, {
			...
			.then(
				async (resp: Response) => {
					...
					const finalMIME = this.MIME || this.resHeaders['content-type'] || 'text/plain';
...
if (globalThis && !('XMLHttpRequest' in globalThis)) {
	// @ts-expect-error we're fine with not a complete patch for our goals
