...
export const transformToSqliteValue = <_Obj extends Record<any, any>>(
	...
	[K in keyof _Obj]: _Obj[K] extends boolean
		...
		: _Obj[K] extends bigint
		? number
		: _Obj[K] extends number | bigint
		? number
		: _Obj[K];
} => {
	return Object.fromEntries(
		Object.entries(obj).map(([key, val]) => {
			if (typeof val !== 'boolean' && !(val instanceof BigInt)) return [key, val];

			if (typeof val === 'bigint') {
				if (BigInt(Number.MAX_SAFE_INTEGER) < val) {
					throw new Error(`BigInt value exceeds maximum safe integer range: ${val}`);
				}
...
export const transformDatabaseSession = (session: SQLiteSessionSchema): SessionSchema => {
	return {
		id: session.id,
