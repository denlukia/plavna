...
import type { Component, ComponentsMap, HastNode, Parser, Plugin, UnistNode } from './types';
...
const transformAriaProps = (node: HastNode) => {
	...
	const ariaProps = Object.keys(node.properties).filter((key) => key.startsWith('aria'));
...
export const allowlist = (tags: Tag[]): Plugin => ({
	renderer: Object.fromEntries(
		htmlAndSvgTags.filter((tag) => !tags.includes(tag)).map((tag) => [tag, null])
	)
});
