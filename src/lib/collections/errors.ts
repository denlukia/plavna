export const ERRORS = {
	AT_LEAST_ONE_TRANSLATION: 'At least one translation is required',
	SOME_TAGS_DONT_EXIST: "Some of specified tags don't exist",
	NO_SUCH_PAGE_TO_CREATE_POST_ON: 'No such page to create article on',
	TOO_MANY_SECTIONS_ON_PAGE: 'Too many sections on page',
	TRANSLATION_FOR_SECTION_NOT_FOUND: 'Translation for section not found',
	PREVIEW_TEMPLATE_NOT_FOUND: 'Such preview template not found',

	COULDNT_LOAD_PREVIEW_COMPONENT: "Couldn't load preview component",
	AT_LEAST_ONE_TITLE: 'At least one title is required',
	UNKNOWN: 'Unknown error occurred',

	IMAGES: {
		INVALID_PROVIDER_CREDS: 'Invalid image provider credentials',
		INVALID_TYPE: 'Invalid image type',
		CANT_DETECT_SIZE: "Can't detect image size"
	}
};
