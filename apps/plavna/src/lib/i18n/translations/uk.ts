...
export const UK = {
	...
	main: {
		landing:
			"## Плавна — безкоштовна платформа для публікації портфоліо та блогів, розроблена Денисом Лук'яненком на Svelte 5. \n## Для авторів це як Medium, де вони можуть налаштувати типографіку, кольори та естетику, а картка кожної статті може бути особливою та інтерактивною. \n## Для читачів це як The Verge, де вони можуть вимкнути нецікаві категорії або переключитися на більш спокійну тему. \n## Для мене це проєкт, де я експериментую зі створенням додатків повністю самостійно: від дизайну, через бекенд, фронтенд і до розгортання, у процесі відшліфовуючи підходи. \n## Plavna орієнтована на лаконічний, високоанімований інтерфейс, тому що я вважаю: анімація — не прикраса, а фундаментальна властивість людського світу.",
	...
	pages_list: {
		...
		color_theme: 'Колір',
		style_theme: 'Стиль',
		typography_markdown_theme: 'Типографіка'
	},
	articles_list: {
		title: 'Мої статті',
		edit: 'Редагувати',
	...
	page_actor: {
		section: {
			...
			add_tags_on_article_creation: 'Ви можете створити ще тегів під час створення статті'
		}
	},
	page: {
		section: {
		...
		previews: {
			...
			families: {
				...
				custom: {
					...
					takes_a_minute: 'Власні передогляди потребують хвилину на оновлення'
				}
			}
		}
	},
	settings: {
		...
		tips: {
			...
			2: 'Створюйте сторінки в Мої > Сторінки або публікуйте на вже створеній там Основній',
			3: 'Додавайте на сторінку секцію і згадайте у її описі теги — статті за згаданими тегами відобразяться у секції. Останні публікації відображатимуться першими'
		}
	}
};
